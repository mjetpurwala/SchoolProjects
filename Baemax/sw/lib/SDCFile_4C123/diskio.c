...
#endif
// PB5 output used for SDC CS
#if SDC_CS_PB5
#define SDC_CS           (*((volatile uint32_t *)0x40005080))
#define SDC_CS_LOW       0           // CS controlled by software
#define SDC_CS_HIGH      0x20
void CS_Init(void){
  ...
  GPIO_PORTB_DR4R_R |= 0x20;            // 4mA drive on outputs
  GPIO_PORTB_PUR_R |= 0x20;             // enable weak pullup on PB5
...
void SSI2_Init(uint32_t CPSDVSR){
  Timer5_Init();                        // initialize Timer5 for 1 ms interrupts
	// DONT FORGET TO DO ME PENDEJA 
  ...
  while((SYSCTL_PRGPIO_R&0x01) == 0){}; // allow time for clock to stabilize
	SYSCTL_RCGCGPIO_R |= 0x02;            // activate clock for Port A
  while((SYSCTL_PRGPIO_R&0x02) == 0){}; // allow time for clock to stabilize
  GPIO_PORTA_DIR_R |= 0x08;             // make PA3 out
  GPIO_PORTB_AFSEL_R |= 0xD0;           // enable alt funct on PB 4,6,7
  GPIO_PORTA_AFSEL_R &= ~0x08;          // disable alt funct on PA3
  GPIO_PORTA_DR4R_R |= 0xC8;            // 4mA drive on outputs
	GPIO_PORTB_DR4R_R |= 0xF0;            // 4mA drive on outputs
  GPIO_PORTA_PUR_R |= 0x08;             // enable weak pullup on PA2,3,4,5
	GPIO_PORTB_PUR_R |= 0xD0;             // enable weak pullup on PA2,3,4,5
  GPIO_PORTA_DEN_R |= 0x08;             // enable digital I/O on PA2,3,4,5
	GPIO_PORTB_DEN_R |= 0xD0;             // enable digital I/O on PA2,3,4,5
  ...
  GPIO_PORTA_AMSEL_R &= ~0x08;          // disable analog functionality on PA2,3,4,5
	GPIO_PORTA_AMSEL_R &= ~0xD0;          // disable analog functionality on PA2,3,4,5
  TFT_CS = TFT_CS_HIGH;                 // disable LCD
  // initialize SSI2
  SYSCTL_RCGCSSI_R |= 0x04;             // activate clock for SSI0
...
// 40 for   400,000 bps slow mode, used during initialization
// 2  for 8,000,000 bps fast mode, used during disk I/O
#define FCLK_SLOW() { SSI2_CPSR_R = (SSI2_CPSR_R&~SSI_CPSR_CPSDVSR_M)+40; }
#define FCLK_FAST() { SSI2_CPSR_R = (SSI2_CPSR_R&~SSI_CPSR_CPSDVSR_M)+2; }
...
#define  SPIxENABLE() {SSI2_Init(40);}
