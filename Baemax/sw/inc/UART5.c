...
void UART5_Init(uint32_t baud){
  ...
  while((SYSCTL_RCGCUART_R & 0x20) == 0){};                 // Wait

  // Enable PORT E clock gating
  
  SYSCTL_RCGCGPIO_R               |= 0x10;
  while((SYSCTL_RCGCGPIO_R & 0x10)!=0x10){};                // allow time to finish activating
  ...
  UART5_CTL_R                     &= ~UART_CTL_UARTEN;    // Clear UART5 enable bit during config
	...
	UART5_IBRD_R = 5000000/baud;   										// IBRD = int(80,000,000 / (16 * baud))
  UART5_FBRD_R = ((64*(5000000%baud))+baud/2)/baud; // FBRD = round(remainder * 64 + 0.5)
