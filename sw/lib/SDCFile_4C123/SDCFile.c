...
// 1  ground
// 2  Vcc +3.3V
// 3  PD6 TFT reset                          //CHANGE
// 4  PD5 TFT data/command                   //CHANGE
// 5  PD7 SDC_CS, active low to enable SDC   //No change, already PD7
// 6  PD4 TFT_CS, active low to enable TFT   //CHANGE
// 7  PD3 MOSI SPI data from microcontroller to TFT or SDC //CHANGE
// 8  PD0 Sclk SPI clock from microcontroller to TFT or SDC  //CHANGE
// 9  PD2 MISO SPI data from SDC to microcontroller //CHANGE
...
static FATFS g_sFatFs, g_sFatFs2;
FIL Handle,Handle2,Handle3;
...
const char inFilename[] = "Sounds/baymax.txt";   // 8 characters or fewer
...
unsigned char readinbuf[500];

void Parser500(void){
	uint32_t i = 0; // 500 buffer index
	uint32_t number = 0;
	
	while(i < 500){
		if (readinbuf[i] == '\n'){
			// we've reached the end of the number and can now concatenate				
			// parsedelements[j] = number;
			//putParsedNumberIntoBuffer(j, number);
			uint8_t bufferHasRoom;
			do {
			bufferHasRoom = addToBuffer(number);
			} while (bufferHasRoom == 0);
			number = 0;
		}
		else if ((readinbuf[i] <= 0x39)&&(readinbuf[i] >= 0x30)){
			// then it is a number and we want to do concatenate
			number *= 10;
			uint8_t digit = readinbuf[i] - '0';
			number += digit; 
		}
		i++;
	}
...
void SDReadIntoBuf(char* inFilename2){
	numbersamples=0;
  UINT successfulreads, successfulwrites;
	uint32_t chunkTracker = 0;
  uint8_t c;
	...
	if (!nowreading){
		MountFresult = f_mount(&g_sFatFs, "", 0);
		Fresult = f_open(&Handle, inFilename2, FA_READ);
	}
  ...
  if(Fresult == FR_OK){
    ...
    while((Fresult == FR_OK) && (successfulreads == 1)){	
			...
			if(c == '{'){
				// skip over the starting stuff and then start reading
				nowreading = 1;
			} 	
			
			else if(nowreading == 1){
				// read the stuff
				readinbuf[chunkTracker] = c;
				
				if(((chunkTracker > 495) && (c == '\n'))){
					break;
				}
...
void SDLCD(char* inFilenameLCD){
  ...
  uint8_t c;
	int x, y;
	uint16_t w = 0, j = 0, k = 0, l = 0;
	uint16_t width = 128;
	uint16_t height = 160;
  MountFresult = f_mount(&g_sFatFs2, "", 0);
  Fresult = f_open(&Handle3, inFilenameLCD, FA_READ);
  if(Fresult == FR_OK){
    Fresult = f_read(&Handle3, &c, 1, &successfulreads);
    x = 127;                              // start in the first column
    y = 159;                             // start in the first row
    while((Fresult == FR_OK) && (successfulreads == 1) && (y >= 0)){
				// the character is printable, so print it
				//first number
				f_read(&Handle3, &w, 1, &successfulreads);
				f_read(&Handle3, &j, 1, &successfulreads);
				f_read(&Handle3, &k, 1, &successfulreads);
				f_read(&Handle3, &l, 1, &successfulreads);

				if((w>= '0') && (w<= '9')){ w= w- '0';}
				else if ((w>= 'A') && (w<= 'F')){ w= w- 'A' + 10;}
				
				if((j >= '0') && (j <= '9')){ j = j - '0';}
				else if ((j >= 'A') && (j <= 'F')){ j = j - 'A' + 10;}
				
				if((k >= '0') && (k <= '9')){ k = k - '0';}
				else if ((k >= 'A') && (k <= 'F')){ k = k - 'A' + 10;}
...
//output this godforsaken pixel
				ST7735_DrawPixel(x,y,w);	
				
        x = x - 1;                      // go to the next column
        if(x < 0){                    // reached the right edge of the screen
          x = 127;                        // go to the first column
          y = y - 1;                   // go to the next row
        }
...
void SDLCD_Transparent(char* inFilenameLCD, uint16_t width, uint16_t height){
  ...
  if(Fresult == FR_OK){
    Fresult = f_read(&Handle3, &c, 1, &successfulreads);
    x = 0;                              // start in the first column
    y = 0;                             // start in the first row
    while((Fresult == FR_OK) && (successfulreads == 1) && (y <= 160)){
				// the character is printable, so print it
				//first number
				f_read(&Handle3, &w, 1, &successfulreads);
				f_read(&Handle3, &j, 1, &successfulreads);
...
//output this godforsaken pixel
        ...
        x = x + 1;                      // go to the next column
        if(x >= 128){                    // reached the right edge of the screen
          x = 0;                        // go to the first column
          y = y + 1;                   // go to the next row
        }
