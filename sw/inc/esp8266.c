...
#ifdef HOTSPOT
    char    eid[32]             = "maj3789";                     // Your EID goes here
    char    ssid[32]            = "THICCBITCH 6789";                      // WiFi Access Point in 5.826
    char    pass[32]            = "hello236";
    char    mqtt_broker[32]     = "10.159.177.113";              // MQTT Broker in EER
    char    mqtt_port[8]        = "1883";
#endif
...
void Reset_8266(void) 
{
    ...
    GPIO_PORTE_DEN_R |= 0x02;             // enable digital I/O on PF3 PF2
  GPIO_PORTE_DATA_R &= ~(0x02);                   // Reset the 8266
  DelayWait10ms(500);           // Wait for 8266 to reset
  GPIO_PORTE_DATA_R = 0x02;                   // Enable the 8266
